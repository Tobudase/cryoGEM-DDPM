...
class ABDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        lr_img = np.array(Image.open(lr_path).convert("L"))  # 改成灰度
...
class ABCDDataset(Dataset):
    def __init__(self, root_lr_img, root_lr_msk, root_hr_img, root_hr_msk):
        ...
        self.lr_img_transforms = A.Compose(
            [
                A.Resize(width=64, height=64, interpolation=Image.BICUBIC),
                A.Normalize(mean=0.5, std=0.5, max_pixel_value=255),  #  改
                ToTensorV2(),
            ])
    ...
    def __getitem__(self, index):
        ...
        lr_img = np.array(Image.open(lr_img).convert("L")) 
        ...
        hr_img = np.array(Image.open(hr_img).convert("L")) 
